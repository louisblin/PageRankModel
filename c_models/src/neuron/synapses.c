...
static inline void _process_fixed_synapses(address_t fixed_region_address, uint32_t time) {
...
static inline void _process_fixed_synapses_page_rank(address_t fixed_region_address, REAL payload) {
    register uint32_t *synaptic_words = synapse_row_fixed_weight_controls(fixed_region_address);
    register uint32_t fixed_synapse = synapse_row_num_fixed_synapses(fixed_region_address);
...
void synapses_do_timestep_update(timer_t time) {
    ...
    for (uint32_t neuron_index = 0; neuron_index < n_neurons;
        ...
        synapse_types_shape_input(&(neuron_synapse_shaping_params[neuron_index]));
...
bool synapses_process_synaptic_row_page_rank(uint32_t time, synaptic_row_t row, REAL payload,
                                    bool write, uint32_t process_id) {
    ...
    if (synapse_row_plastic_size(row) > 0) {
        return synapses_process_synaptic_row(time, row, write, process_id);
    }

    _process_fixed_synapses_page_rank(fixed_region_address, payload);
    return true;
}
